...
def decode_jwt_token(request: Request) -> Dict[str, Any]:
    ...
    if not auth_header.startswith("Bearer "):
        raise HTTPException(
            status_code=401, detail="Not authenticated - Missing or invalid Authorization header"
        )
...
def load_tier_info(user_id: str, session: Session) -> Dict[str, Any]:
    ...
    try:
        ...
        if row:
            return {"tier_slug": row[0], "tier_level": row[1], "status": row[2]}
        else:
            # Default to member tier
            return {"tier_slug": "member", "tier_level": 1, "status": "active"}
    except Exception as e:
