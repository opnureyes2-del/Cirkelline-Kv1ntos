"""
Cirkelline Notion Integration Helpers
======================================
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            for db_item in databases:
                try:
                    # Get full schema
                    schema = get_database_schema(notion, db_item["id"])
                    ...
                    logger.info(f"   ├─ {db_item['title'][:40]}: {db_type}")
                    ...
                    logger.error(f"Error processing database {db_item['title']}: {e}")
                    continue
