...
const EmphasizedText = ({ className, ...props }: EmphasizedTextProps) => (
  <em
    className={cn(className, PARAGRAPH_SIZES.body, 'font-semibold')}
    {...filterProps(props)}
  />
)
...
const AnchorLink = ({ className, ...props }: AnchorLinkProps) => (
  <a
    className={cn(className, PARAGRAPH_SIZES.body, 'cursor-pointer underline font-mono')}
    target="_blank"
...
const Heading1 = ({ className, ...props }: HeadingProps) => (
  <h1 className={cn(className, PARAGRAPH_SIZES.body)} {...filterProps(props)} />
)

const Heading2 = ({ className, ...props }: HeadingProps) => (
  <h2 className={cn(className, PARAGRAPH_SIZES.body)} {...filterProps(props)} />
)

const Heading3 = ({ className, ...props }: HeadingProps) => (
  <h3 className={cn(className, PARAGRAPH_SIZES.body)} {...filterProps(props)} />
)

const Heading4 = ({ className, ...props }: HeadingProps) => (
  <h4 className={cn(className, PARAGRAPH_SIZES.body)} {...filterProps(props)} />
)
...
const TableHead = ({ className, ...props }: TableHeaderProps) => (
  <thead
    className={cn(
      className,
      PARAGRAPH_SIZES.body,
      'rounded-md border-b border-border bg-transparent p-2 text-left font-[600]'
    )}
    {...filterProps(props)}
  />
)
...
const TableHeadCell = ({ className, ...props }: TableHeaderCellProps) => (
  <th
    className={cn(className, PARAGRAPH_SIZES.body, 'p-2 font-[600]')}
    {...filterProps(props)}
  />
)

const TableBody = ({ className, ...props }: TableBodyProps) => (
  <tbody className={cn(className, PARAGRAPH_SIZES.body)} {...filterProps(props)} />
)

const TableRow = ({ className, ...props }: TableRowProps) => (
  <tr
    className={cn(className, PARAGRAPH_SIZES.body, 'border-b border-border last:border-b-0')}
    {...filterProps(props)}
  />
)
...
const TableCell = ({ className, ...props }: TableCellProps) => (
  <td
    className={cn(className, PARAGRAPH_SIZES.body, 'whitespace-nowrap p-2 font-[400]')}
    {...filterProps(props)}
  />
)
